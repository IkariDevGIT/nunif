...
2026-06-12: 最新の更新では`Visual C++ 再頒布可能パッケージ`が古いとエラーが発生しますので上記のバージョンをインストールしてください。
...
Windowsではファイルパスの最大長に260文字の制限があります。
nunif-windowsを深いパスに配置した場合、いくつかのファイルがこの制限によってエラーになることがあります。  
この場合、よくあるエラーは`No module named '...'`です。過度に長いパスにあるファイルが読み込めないのでこのエラーが起こります。
